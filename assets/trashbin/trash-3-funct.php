...
add_filter('template_include', function (string $template): string {
    if (isset($_GET['spoke_debug'])) {
        echo '<pre style="background:red;color:white;padding:20px;font-size:14px;position:fixed;top:0;left:0;right:0;z-index:99999;">';
        ...
        if (function_exists('tutor_utils')) {
            echo "\nTUTOR DASHBOARD OPTION (get_option): " . get_option('tutor_dashboard_page_id') . "\n";
            echo "TUTOR DASHBOARD OPTION (tutor_utils): " . tutor_utils()->get_option('dashboard') . "\n";
        }
        echo '</pre>';
    }
    return $template;
...
// ─────────────────────────────────────────────────────────────────
// AUTO-RESOLVE: Wire the Navigation block to the 'primary' menu
//
// Without this the wp:navigation block with no "ref" falls back to
// showing ALL pages instead of your assigned menu.
// This filter finds the WP nav menu assigned to 'primary' location
// and injects its ID into the block so it renders the right menu
// automatically on every fresh site install — zero manual config.
// ─────────────────────────────────────────────────────────────────

add_filter('render_block_core/navigation', function (string $html, array $block): string {
    // Already pointing at a specific menu — leave it alone
    if (!empty($block['attrs']['ref'])) {
        return $html;
    }
    // Only act on primary-location or unspecified nav blocks in the header
    $location = $block['attrs']['__experimentalLocation'] ?? '';
    if ($location !== 'primary' && $location !== '') {
        return $html;
    }
    // Find the menu assigned to the 'primary' theme location
    $locations = get_nav_menu_locations();
    if (empty($locations['primary'])) {
        return $html; // No menu assigned yet; WordPress shows all-pages fallback
    }
    $menu = wp_get_nav_menu_object($locations['primary']);
    if (!$menu) {
        return $html;
    }
    // Re-render with the correct menu ref injected
...
// ─────────────────────────────────────────────────────────────────
// 5. FIX: WOOCOMMERCE CART & CHECKOUT — LAYOUT FIX
// ─────────────────────────────────────────────────────────────────
add_action('wp_head', function (): void {
    if (! (is_cart() || is_checkout() || is_account_page())) {
        return;
    ...
    echo '<style>
        /* Force the WooCommerce container to match your theme constraints */
        ...
        .woocommerce-account main.is-layout-constrained {
            max-width: 1280px !important; /* Matches your header.html */
...
add_action('template_redirect', function (): void {
    // Primary: WooCommerce shop page
    if (function_exists('is_shop') && is_shop()) {
        wp_safe_redirect(home_url('/courses/'), 301);
        exit;
    }
    // Fallback: catch /shop/ slug directly
    global $wp;
...
// ─────────────────────────────────────────────────────────────────
// EMPTY CART — inject "Browse Courses" CTA
// CSS in woocommerce-overrides.css handles hiding the product grid
// ─────────────────────────────────────────────────────────────────
add_filter('render_block', function (string $content, array $block): string {
    ...
    // Inject the button right after the empty cart title heading
    $content = preg_replace(
        '/(<h2[^>]*class="[^"]*wc-block-cart__empty-cart__title[^"]*"[^>]*>.*?<\/h2>)/is',
        '$1' . $btn,
        $content,
        1
    );
...
// ─────────────────────────────────────────────────────────────────
// REDIRECT course-category taxonomy pages → /courses/?cat=slug
// ─────────────────────────────────────────────────────────────────
add_action('template_redirect', function (): void {
    ...
    wp_safe_redirect(
        add_query_arg('cat', $term->slug, home_url('/courses/')),
        302  // 302 so it stays dynamic if categories change
    );
    exit;
});
...
add_filter('template_include', function(string $template): string {
    ...
    $tutor_plugin_path = WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . 'tutor';
    $is_tutor = (strpos($template, $tutor_plugin_path) !== false);

    if (!$is_tutor) {
        return $template;
    }

    // Stop here and show us everything
    $page_html_path = get_template_directory() . '/templates/page.html';
    $content = file_exists($page_html_path) ? file_get_contents($page_html_path) : 'FILE NOT FOUND';
    ...
    die('
        IS_TUTOR: true<br>
        PLUGIN_DIR: ' . WP_PLUGIN_DIR . '<br>
        TEMPLATE: ' . $template . '<br>
        TUTOR_PATH_CHECK: ' . $tutor_plugin_path . '<br>
        STRPOS RESULT: ' . var_export(strpos($template, $tutor_plugin_path), true) . '<br>
        PAGE_HTML_PATH: ' . $page_html_path . '<br>
        FILE_EXISTS: ' . var_export(file_exists($page_html_path), true) . '<br>
        CONTENT_LENGTH: ' . strlen($content) . '<br>
        CONTENT_FIRST_100: ' . htmlspecialchars(substr($content, 0, 100)) . '<br>
        CANVAS_PATH: ' . $canvas . '<br>
        CANVAS_EXISTS: ' . var_export(file_exists($canvas), true) . '<br>
    ');
