...
function spoke_render_single_course(): void {
	...
	if ( $wc_linked && $wc_linked->is_purchasable() && $wc_linked->is_in_stock() ) {
		$enrol_url  = $wc_linked->add_to_cart_url();
		$enrol_cls  = 'ajax_add_to_cart add_to_cart_button';
		$enrol_data = 'data-quantity="1" data-product_id="' . esc_attr( $wc_linked->get_id() ) . '" data-product_sku="' . esc_attr( $wc_linked->get_sku() ) . '"';
	} else {
	...
	echo '<span style="font-size:13px;color:rgba(255,255,255,0.75);">' . ( $students > 0 ? number_format( $students ) : '0' ) . ' Students</span>';
	echo '</div>';
	...
	// Buttons
	$enrol_display     = $in_cart ? 'none'  : 'block';
	$view_cart_display = $in_cart ? 'block' : 'none';
	echo '<div style="padding:12px 20px 0;" id="sc-btn-wrap">';
	echo '<a id="sc-enrol-btn" href="' . esc_url( $enrol_url ) . '" ' . $enrol_data . ' class="' . esc_attr( $enrol_cls ) . '" style="display:' . $enrol_display . ';width:100%;text-align:center;font-family:\'Inter\',sans-serif;font-size:18px;font-weight:800;padding:16px 0;background:#fdaf2e;color:#6b4500;border-radius:8px;text-decoration:none;box-sizing:border-box;box-shadow:0 4px 12px rgba(253,175,46,0.3);">Enrol Now</a>';
	echo '<a id="sc-view-cart-btn" href="' . esc_url( $cart_url ) . '" style="display:' . $view_cart_display . ';width:100%;text-align:center;font-family:\'Inter\',sans-serif;font-size:18px;font-weight:800;padding:16px 0;background:#1A3C6E;color:#fff;border-radius:8px;text-decoration:none;box-sizing:border-box;">View Cart →</a>';
	echo '</div>';
	...
	echo '</div></div>';
	if ( $tutor_active ) {
		do_action( 'tutor_course/single/reviews' );
	}
	echo '</div>';
	...
	if ( $rel->have_posts() ) {
		...
		echo '<div style="max-width:1280px;margin:0 auto;">';
		echo '<div style="display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:16px;margin-bottom:32px;"><h2 style="font-size:24px;font-weight:700;color:#1A3C6E;margin:0;">Continue Your Learning</h2><a href="' . esc_url( home_url('/courses/') ) . '" style="font-size:14px;font-weight:700;color:#1A3C6E;text-decoration:none;">View All Courses →</a></div>';
		echo '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:24px;">';
		while ( $rel->have_posts() ) {
			...
			echo '<article style="background:#fff;border-radius:12px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 2px 8px rgba(0,0,0,0.06);">';
			echo '<div style="height:170px;overflow:hidden;"><a href="' . get_permalink() . '" style="display:block;width:100%;height:100%;" tabindex="-1" aria-hidden="true">';
			if ( $rthumb ) {
				echo '<img src="' . esc_url( $rthumb ) . '" alt="' . esc_attr( get_the_title() ) . '" style="width:100%;height:100%;object-fit:cover;" loading="lazy" width="400" height="170">';
			} else {
				echo '<div style="width:100%;height:100%;background:linear-gradient(135deg,#1A3C6E,#1A1A2E);display:flex;align-items:center;justify-content:center;"><svg style="width:40px;height:40px;opacity:0.2;" fill="white" viewBox="0 0 24 24"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"/></svg></div>';
			}
			echo '</a></div>';
			echo '<div style="padding:20px;display:flex;flex-direction:column;flex:1;gap:8px;">';
			if ( $rcat ) {
				echo '<span style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;background:rgba(26,60,110,0.08);color:#1A3C6E;padding:2px 8px;border-radius:4px;align-self:flex-start;">' . esc_html( $rcat ) . '</span>';
			}
			echo '<h3 style="font-size:15px;font-weight:700;color:#1A3C6E;margin:0;line-height:1.3;"><a href="' . get_permalink() . '" style="color:inherit;text-decoration:none;">' . get_the_title() . '</a></h3>';
			echo '<div style="display:flex;align-items:center;gap:6px;">' . spoke_render_stars( $ravg ) . '<span style="font-size:12px;font-weight:700;color:#1A3C6E;">' . number_format( $ravg, 1 ) . '</span></div>';
			echo '<div style="display:flex;align-items:center;justify-content:space-between;margin-top:auto;padding-top:12px;border-top:1px solid rgba(0,0,0,0.06);"><span style="font-size:18px;font-weight:900;color:#1A3C6E;">£' . number_format( $rprice, 2 ) . '</span><a href="' . get_permalink() . '" style="display:inline-flex;align-items:center;height:36px;padding:0 16px;background:#1A3C6E;color:#fff;border-radius:8px;font-size:13px;font-weight:700;text-decoration:none;">Learn More</a></div>';
			echo '</div></article>';
		}
		wp_reset_postdata();
	...
	// Final scrub — catch anything wpautop may have injected via the_content()
	$html = preg_replace( '/<br\s*\/?>/i', '', $html );
	$html = preg_replace( '/<p>(\s|&nbsp;)*<\/p>/i', '', $html );
...
#sc-btn-wrap .added_to_cart{display:none!important;}
...
function scShare(url,title){
  function copyFallback(u){
    ...
    if(l){l.textContent='Copied!';setTimeout(function(){l.textContent='Share';},2000);}
  }
...
(function(){
  ...
  function showViewCart(){if(enrolBtn)enrolBtn.style.display='none';if(viewCartBtn)viewCartBtn.style.display='block';}
  if(typeof jQuery!=='undefined'){
    jQuery(document.body).on('added_to_cart',function(e,fragments,cart_hash,$btn){if($btn&&enrolBtn&&($btn[0]===enrolBtn||($btn.is&&$btn.is('#sc-enrol-btn')))){showViewCart();}});
    jQuery(document.body).on('wc_cart_button_updated',function(e,$btn){if($btn&&enrolBtn&&$btn[0]===enrolBtn){showViewCart();}});
  }
  if(enrolBtn){new MutationObserver(function(mutations){mutations.forEach(function(m){if(m.type==='attributes'&&m.attributeName==='class'){if(enrolBtn.classList.contains('added')||enrolBtn.classList.contains('added_to_cart')){showViewCart();}}});}).observe(enrolBtn,{attributes:true});}
})();
</script>
JS;
}