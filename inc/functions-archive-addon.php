...
function spoke_courses_to_json(WP_Query $loop): array
{
    ...
    if ($loop->have_posts()) {
        while ($loop->have_posts()) {
            ...
            // ── Rating & students — fake data takes priority ───────────
...
?>
    ...
    <script>
        (function() {
            ...
            function cacheKey(p) {
                return CACHE_PREFIX + [p.cat, p.sort, p.search, p.price, p.level, p.page].join('|');
            }
            ...
            function renderPagination(maxPages, current) {
                ...
                function makeBtn(label, page, disabled, active) {
                    ...
                    if (!disabled) el.addEventListener('click', function() {
                        goToPage(page);
                    });
                    return el;
                }
            ...
            function goToPage(page) {
                state.page = page;
                fetchAndRender();
                document.querySelector('.spoke-archive-wrap').scrollIntoView({
                    behavior: 'smooth',
                    block: 'start'
                });
            }
            ...
            document.querySelectorAll('.spoke-level-check').forEach(function(r) {
                r.addEventListener('change', function() {
                    var checked = [];
                    document.querySelectorAll('.spoke-level-check:checked').forEach(function(cb) {
            ...
            // ── READ URL PARAM AND PRE-APPLY CATEGORY FILTER ──────────────
            (function() {
                ...
                if (catSlug) {
                    ...
                    // Tick the matching category dropdown option
                    if (catEl) {
                        var opts = catEl.querySelectorAll('option');
                        opts.forEach(function(opt) {
                            if (opt.value === catSlug) {
                                opt.selected = true;
                            }
                        });
                    }

                    // Also highlight the matching category pill (if visible)
                    document.querySelectorAll('.spoke-cat-pill').forEach(function(pill) {
                        if (pill.dataset.slug === catSlug) {
                    ...
                    // Fetch filtered results instead of showing the server-baked all-courses data
                    fetchAndRender();
                } else {
                    // No filter — use the fast server-baked initial data
                    renderData(INITIAL);
                }
                bindWooEvents();
            }());
